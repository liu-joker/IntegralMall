      ...
      <scroller lock-x @on-scroll-bottom="onScrollBottom" ref="scrollerBottom2" :scroll-bottom-offst="300" v-if="!noDataShow">
        ...
        <div class="item" v-for="(x,index) in orderList" :key="index" @click="openOrderDetails(x)">
          <div class="headTitle">
            ...
            <div class="right" :class="x.expressStatus != 2?'default':''">
              {{expressStatus(x)}}
            </div>
          </div>
          <div class="O_content">
  ...
  export default {
    ...
    methods: {
        ...
        if (x.expressStatus == 2) {
          this.$vux.confirm.show({
            ...
            onConfirm: () => {
              ...
              this.$axiosApi.finishOrder(orderNum).then(res => {
                if (res.code == 200) {
                  this.$vux.toast.show({
                    text: '操作成功',
                    width: 'auto'
      ...
      getData() {
        ...
        this.$axiosApi.findUserShopOrder(this.pageNum, this.pageSize).then(res => {
